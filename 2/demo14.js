...
function animate(elapsedMilliseconds) {
    ...
    for (var index = 0; index < coordinates.length; index += 1) {
        var offset = Math.max(0, 100 - coordinates[index].mouseDistance / 100);
        svgElement.updateCircle(
            circles[index],
            {
                ...
                r : (Math.sin(pi * coordinates[index].multiplier) + 1.5) * (1 + coordinates[index].mouseDistance / 10)
            }
        );
        svgElement.updateCircle(
            circles[index + circleCount],
            {
                cx : coordinates[index].x - Math.cos(coordinates[index].mouseRad) * offset,
                cy : coordinates[index].y - Math.sin(coordinates[index].mouseRad) * offset,
                r : (Math.sin(pi * coordinates[index].multiplier) + 1.5) * (1 + coordinates[index].mouseDistance / 20)
            }
        );
    }
